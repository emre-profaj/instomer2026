...
export const sendTemplateMessage = async (req, res) => {
    try {
        ...
        if (template.headerType && ['IMAGE', 'VIDEO', 'DOCUMENT'].includes(template.headerType)) {
            ...
            // Only send the header component if it's NOT a temporary Meta CDN URL
            // If we omit it, Meta automatically uses the media approved with the template.
            if (rawMediaUrl && !rawMediaUrl.includes('scontent.whatsapp.net') && !rawMediaUrl.includes('scontent.cdninstagram.com')) {
                ...
                console.log('📎 Header component omitted to use Meta default media (avoiding 403 error on scontent URLs)');
            }
        }
...
export const sendTemplateDynamic = async (req, res) => {
    try {
        ...
        if (template.headerType && ['IMAGE', 'VIDEO', 'DOCUMENT'].includes(template.headerType)) {
            ...
            if (rawUrl && !rawUrl.includes('scontent.whatsapp.net') && !rawUrl.includes('scontent.cdninstagram.com')) {
                ...
                if (mediaUrl) {
                    const needsUpload = mediaUrl.includes('drive.usercontent.google.com') || mediaUrl.includes('drive.google.com');
                    let headerParam;
