...
const CeoReport = () => {
    ...
    return (
        <div className="ceo-report">
            ...
            {/* ── İkon Projeler Facebook Lead Tablosu ── */}
            {currentWorkspace?.id === IKON_WS && leadsByForm && leadsByForm.byForm?.length > 0 && (
                    ...
                    <div style={{
                        ...
                        <div>
                            <div style={{ fontSize: 15, fontWeight: 700, color: '#111827' }}>
                                Facebook Lead — Proje Bazlı Dağılım
                            </div>
                            <div style={{ fontSize: 12, color: '#6b7280' }}>
                                Toplam {leadsByForm.total?.toLocaleString('tr-TR')} lead — form adına göre
                            </div>
                        </div>
                    </div>
                    <div style={{
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                                <tr style={{ background: '#f8fafc', borderBottom: '1.5px solid #e5e7eb' }}>
                                    ...
                                    <th style={{ padding: '11px 18px', textAlign: 'left', fontSize: 12, fontWeight: 600, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.4px' }}>Form / Proje Adı</th>
                            ...
                            <tbody>
                                {leadsByForm.byForm.map((row, i) => {
                                    const pct = leadsByForm.total > 0 ? ((row.count / leadsByForm.total) * 100).toFixed(1) : 0;
                                    const colors = ['#6366f1','#8b5cf6','#f97316','#22c55e','#3b82f6','#ec4899'];
                                    const color = colors[i % colors.length];
                                    return (
                                        <tr key={i}
                                            style={{ borderBottom: i < leadsByForm.byForm.length - 1 ? '1px solid #f3f4f6' : 'none', transition: 'background .1s' }}
                                        ...
                                        >
                                            ...
                                            <td style={{ padding: '13px 18px', textAlign: 'right' }}>
                                                <span style={{ display: 'inline-block', background: color + '18', color, borderRadius: 7, padding: '3px 10px', fontWeight: 700, fontSize: 14 }}>
                            ...
                            <tfoot>
                                <tr style={{ background: '#f8fafc', borderTop: '1.5px solid #e5e7eb' }}>
                                    ...
                                    <td style={{ padding: '11px 18px', textAlign: 'right', fontSize: 14, fontWeight: 700, color: '#6366f1' }}>{leadsByForm.total?.toLocaleString('tr-TR')}</td>
